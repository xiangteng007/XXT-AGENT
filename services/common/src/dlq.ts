...
export class DLQManager {
    ...
    async replayFromDLQ(
        ...
        return new Promise((resolve) => {
            ...
            const messageHandler = async (message: Message) => {
                ...
                try {
                    const dlqMessage: DLQMessage = JSON.parse(message.data.toString());
                    ...
                    const originalData = typeof dlqMessage.data === 'object' && dlqMessage.data !== null
                        ? dlqMessage.data as Record<string, unknown>
     ...
     */
    async getDLQStats(originalTopic: string): Promise<{
        topic: string;
...
export function withDLQ<T>(
    ...
    options: RetryableProcessorOptions
): (event: T & { __retryCount?: number }) => Promise<void> {
    const { dlqManager, topicName, maxRetries = 3 } = options;

    return async (event: T & { __retryCount?: number }): Promise<void> => {
 ...
 */
export async function runReplayCLI(args: string[]): Promise<void> {
