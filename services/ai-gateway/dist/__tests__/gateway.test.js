...
const providerReady = { google: true, openai: false, anthropic: false };
function createTestApp(apiKeys = []) {
...
describe('AI Gateway', () => {
    ...
    describe('GET /ai/models', () => {
        ...
        it('should return available models filtered by provider readiness', async () => {
            const res = await (0, supertest_1.default)(app).get('/ai/models');
            expect(res.status).toBe(200);
        ...
        it('should list unavailable provider models separately', async () => {
            ...
            expect(res.body.unavailable.every((m) => m.available === false)).toBe(true);
        });
        it('should mark default model correctly', async () => {
            const res = await (0, supertest_1.default)(app).get('/ai/models');
            const defaultModel = res.body.models.find((m) => m.isDefault);
