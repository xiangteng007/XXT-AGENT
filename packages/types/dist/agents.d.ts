...
export type AgentId = 'accountant' | 'guardian' | 'finance' | 'scout' | 'zora' | 'lex' | 'nova' | 'titan' | 'lumi' | 'rusty' | 'invest' | 'sage';
...
/** 已知的跨 Agent 協作動作 */
export type CollaborationAction = 'SCOUT_REQUEST_CONTRACT' | 'GUARDIAN_PREMIUM_BOOKING' | 'ZORA_DONATION_BOOKING' | 'RECONCILE_REQUEST' | 'RECONCILE_RESPONSE';
/** 協作結果 */
...
export type LedgerCategory = 'engineering_payment' | 'advance_payment' | 'design_fee' | 'consulting_fee' | 'material_rebate' | 'other_income' | 'salary' | 'freelance' | 'rental_income' | 'investment_gain' | 'allowance' | 'material' | 'labor' | 'subcontract' | 'equipment' | 'overhead' | 'insurance' | 'tax_payment' | 'utilities' | 'rent' | 'office_supply' | 'entertainment' | 'transportation' | 'professional_service' | 'other_expense' | 'medical' | 'education' | 'life_insurance' | 'house_rent' | 'family_living';
...
export type PolicyCategory = 'car_insurance' | 'pli' | 'workers_comp' | 'employers_liability' | 'equipment' | 'professional_liability' | 'life_term' | 'life_whole' | 'accident' | 'medical' | 'critical_illness' | 'disability' | 'cancer' | 'fire' | 'earthquake' | 'vehicle' | 'travel' | 'volunteer_group' | 'uav_hull' | 'uav_tpl' | 'other';
...
export type LoanCategory = 'mortgage' | 'car_loan' | 'business_loan' | 'credit_line' | 'equipment_loan' | 'micro_loan' | 'student_loan' | 'personal_loan' | 'policy_loan' | 'other';
...
export type ReconciliationStatus = 'MATCHED' | 'DISCREPANCY' | 'MISSING_LEDGER' | 'MISSING_LOAN' | 'AMOUNT_MISMATCH';
...
export type WriteRequestStatus = 'QUEUED' | 'PROCESSING' | 'DELIVERED' | 'RETRYING' | 'DEAD_LETTER' | 'ALREADY_PROCESSED';
