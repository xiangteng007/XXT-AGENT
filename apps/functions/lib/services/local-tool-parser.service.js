...
exports.SENSITIVE_TOOL_PATTERNS = void 0;
exports.mightContainToolIntent = mightContainToolIntent;
exports.parseLocalToolCall = parseLocalToolCall;
exports.isSensitiveTool = isSensitiveTool;
const v2_1 = require("firebase-functions/v2");
const local_inference_service_1 = require("./local-inference.service");
// ================================
// Intent Detection System Prompt
// ================================
function buildParserSystemPrompt(today) {
 ...
 */
exports.SENSITIVE_TOOL_PATTERNS = [
    // Financial recording
...
function mightContainToolIntent(message) {
 ...
 */
async function parseLocalToolCall(message, agentId = 'butler') {
    ...
    try {
        const raw = await (0, local_inference_service_1.ollamaChat)([
            { role: 'system', content: systemPrompt },
            { role: 'user', content: message },
        ], model, { temperature: 0.1, num_predict: 256 });
        ...
        try {
            ...
            v2_1.logger.warn(`[LocalToolParser] JSON parse failed, treating as no-tool. Raw: "${raw.slice(0, 200)}"`);
            return { toolCall: null, source: 'local' };
        }
        if (!parsed.tool) {
            v2_1.logger.info(`[LocalToolParser] No tool intent detected for message: "${message.slice(0, 60)}"`);
            return { toolCall: null, source: 'local' };
        }
        const toolCall = {
            name: parsed.tool,
        ...
        v2_1.logger.info(`[LocalToolParser] ✅ Parsed tool=${toolCall.name} args=${JSON.stringify(toolCall.args)} via local/${model}`);
        return { toolCall, source: 'local' };
    }
    catch (err) {
        if (err instanceof local_inference_service_1.OllamaUnavailableError) {
            v2_1.logger.warn(`[LocalToolParser] Ollama offline — sensitive tool ops blocked. Reason: ${err.message}`);
            return { toolCall: null, source: 'offline' };
        }
        // Unexpected error — treat as no-tool to avoid crashing
        v2_1.logger.error('[LocalToolParser] Unexpected error during parse:', err);
        return { toolCall: null, source: 'local' };
    }
}
// ================================
...
//# sourceMappingURL=local-tool-parser.service.js.map