...
exports.calculateQuantMetrics = calculateQuantMetrics;
exports.formatQuantForPrompt = formatQuantForPrompt;
// ─────────────────────────────────────────
// Core Math Utilities
// ─────────────────────────────────────────
function mean(arr) {
...
function realizedVol(prices, tradingDaysPerYear = 252) {
    if (prices.length < 2)
        return 0;
    const logReturns = [];
...
/** SMA drift */
function smaDeviation(prices, period = 20) {
...
function kMeansClusters(prices, k = 3) {
    if (prices.length < k)
        return [...prices];
...
// Main calculator
// ─────────────────────────────────────────
function calculateQuantMetrics(symbol, candles) {
    ...
    if (hurst > 0.6 && drift > 0)
        score += 20;
    else if (hurst > 0.6 && drift < 0)
        score -= 20;
...
// Prompt Formatter
// ─────────────────────────────────────────
function formatQuantForPrompt(m) {
    return [
        `【量化指標 ${m.symbol}】`,
...
//# sourceMappingURL=quant-engine.service.js.map