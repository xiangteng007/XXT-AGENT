...
exports.getButlerContext = getButlerContext;
const admin = __importStar(require("firebase-admin"));
const db = admin.firestore();
/**
 * Fetch aggregated user data from Firestore for AI context
 */
async function getButlerContext(userId) {
    const context = {};
    ...
    if (finance.status === 'fulfilled')
        context.finance = finance.value;
    if (vehicle.status === 'fulfilled')
        context.vehicle = vehicle.value;
    if (calendar.status === 'fulfilled')
        context.calendar = calendar.value;
    if (preferences.status === 'fulfilled')
        context.preferences = preferences.value;
    return context;
}
...
async function getVehicleSummary(userId) {
...
async function getCalendarSummary(userId) {
    ...
    if (todaySnap.empty && weekSnap.empty)
        return undefined;
    return {
...
//# sourceMappingURL=butler-data.service.js.map