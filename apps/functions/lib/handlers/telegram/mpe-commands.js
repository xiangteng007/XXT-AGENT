...
exports.handleSignalCommand = handleSignalCommand;
exports.handleMpeCommand = handleMpeCommand;
exports.handlePredictCommand = handlePredictCommand;
exports.handleBacktestCommand = handleBacktestCommand;
const v2_1 = require("firebase-functions/v2");
const signal_generator_service_1 = require("../../services/mpe/signal-generator.service");
const macro_data_service_1 = require("../../services/mpe/macro-data.service");
const osint_scanner_service_1 = require("../../services/mpe/osint-scanner.service");
const market_data_service_1 = require("../../services/mpe/market-data.service");
const monte_carlo_service_1 = require("../../services/mpe/monte-carlo.service");
const firestore_1 = require("firebase-admin/firestore");
const db = (0, firestore_1.getFirestore)();
// ─────────────────────────────────────────
// /signal — Active signals list
// ─────────────────────────────────────────
async function handleSignalCommand(ctx) {
    await ctx.reply('🔍 查詢中...');
    try {
        const signals = await (0, signal_generator_service_1.getActiveSignals)();
        if (signals.length === 0) {
            await ctx.reply('📭 目前沒有符合門檻的有效訊號（R/R ≥ 2.5 且信心 ≥ 55%）');
            return;
        }
        for (const signal of signals.slice(0, 3)) {
            await ctx.reply((0, signal_generator_service_1.formatSignalForTelegram)(signal), { parse_mode: 'Markdown' });
        }
        if (signals.length > 3) {
        ...
        v2_1.logger.error('[MPE Telegram] /signal error:', err);
...
// ─────────────────────────────────────────
async function handleMpeCommand(ctx) {
    await ctx.reply('📊 查詢 MPE 系統狀態...');
    try {
        const [macro, sentiment, signalCount] = await Promise.all([
            (0, macro_data_service_1.fetchMacroSnapshot)(),
            (0, osint_scanner_service_1.runOsintScan)(),
            (0, signal_generator_service_1.getActiveSignals)().then(s => s.length),
        ]);
        ...
        v2_1.logger.error('[MPE Telegram] /mpe error:', err);
...
async function handlePredictCommand(ctx) {
    ...
    if (!symbol) {
        await ctx.reply('使用方式：`/predict 2330`（台積電）或 `/predict TSLA`\n強制重生成：`/predict force 2330`', { parse_mode: 'Markdown' });
        return;
    }
    ...
    try {
        ...
        if (!forceRegen) {
            const existing = await (0, signal_generator_service_1.getActiveSignals)(symbol);
            if (existing.length > 0) {
                await ctx.reply((0, signal_generator_service_1.formatSignalForTelegram)(existing[0]), { parse_mode: 'Markdown' });
                await ctx.reply(`ℹ️ 以上為快取訊號（有效期內），使用 \`/predict force ${symbol}\` 強制重新生成`, { parse_mode: 'Markdown' });
                return;
            }
        }
        ...
        let ohlcv;
        try {
            ohlcv = await (0, market_data_service_1.fetchOHLCV)(symbol);
        }
        catch (fetchErr) {
            v2_1.logger.warn(`[MPE Telegram] OHLCV fetch failed for ${symbol}:`, fetchErr instanceof Error ? fetchErr.message : fetchErr);
            await ctx.reply(`⚠️ 無法取得 ${symbol} 的 K線資料\n\n` +
                `請確認代號是否正確：\n• 台股：4-6位數字（如 2330、0050）\n• 美股：英文代號（如 TSLA、AAPL）\n\n` +
                `使用 /signal 查看已有分析的標的列表。`);
            return;
        }
        const closePrices = (0, market_data_service_1.extractClosePrices)(ohlcv);
        const annualizedVol = (0, market_data_service_1.calcAnnualizedVolatility)(closePrices);
        const annualizedDrift = (0, market_data_service_1.calcAnnualizedDrift)(closePrices);
        ...
        try {
            const signal = await (0, signal_generator_service_1.generateSignal)({
                symbol,
                currentPrice: ohlcv.latestClose,
            ...
            if (signal) {
                ...
                await ctx.reply(`⚠️ ${symbol} 訊號未達門檻（R/R < 2.5 或信心 < 55%）\n\n` +
                    `以上蒙特卡洛預覽仍可供參考。`);
            }
        }
        catch (sigErr) {
            v2_1.logger.error(`[MPE Telegram] Signal generation failed for ${symbol}:`, sigErr);
            await ctx.reply(`⚠️ 完整訊號生成失敗（Ollama 可能離線）\n\n以上蒙特卡洛預覽仍可供參考。\n使用 /mpe 查看系統狀態。`);
        }
    }
    catch (err) {
        v2_1.logger.error(`[MPE Telegram] /predict ${symbol} error:`, err);
...
// ─────────────────────────────────────────
async function handleBacktestCommand(ctx) {
    const text = ctx.message?.text ?? '';
    const parts = text.trim().split(/\s+/);
    ...
    try {
        ...
        v2_1.logger.error('[MPE Telegram] /backtest error:', err);
        await ctx.reply('❌ 回測查詢失敗');
    }
}
//# sourceMappingURL=mpe-commands.js.map