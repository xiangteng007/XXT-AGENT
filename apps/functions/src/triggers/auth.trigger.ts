...
export const onUserCreated = auth.user().onCreate(async (user) => {
    ...
    if (isSuperadminEmail(email)) {
        logger.info(`🔐 Auto-provisioning superadmin for: ${email}`);
        ...
        try {
            ...
            logger.info('✅ Set globalRole: superadmin');
            ...
            logger.info('✅ Created Firestore user document');

            logger.info(`🎉 Superadmin provisioned for: ${email}`);
        } catch (error) {
            logger.error('Error provisioning superadmin:', error);
        }
    } else {
        // Regular user
        await db.collection('users').doc(user.uid).set({
        ...
        logger.info(`✅ Created regular user document for: ${email}`);
    }
});
